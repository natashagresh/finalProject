  ...
  $('#germany').on('click', function(event){
    $.ajax({
      ...
      $('#categoriesContainer').on('click', 'p', function() {
        ...
        $.ajax({
          url: '/api/getplaylistsgermany/' + category,
          type: "GET",
          dataType: 'json'
        }).done(function(results){
          console.log(results);
      ...
      $categoriesTitle.text('Choose from one of the top Categories in Germany to hear top Playlists');
