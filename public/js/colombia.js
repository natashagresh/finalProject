console.log('connectedcolombia');

$(document).ready(function(){
  
  var $body = $('body');

   $('#colombia').on('click', function(event){
    $.ajax({
      url: '/api/releasescolombia',
      type: 'GET',
      dataType: 'json'
    }).done(function(results){
        var releasesTitle = '<h2>New Releases in Colombia</h2>';
  ...
  $('#colombia').on('click', function(event){
    $.ajax({
      ...
      $('#categoriesContainer').on('click', 'p', function() {
        ...
        $.ajax({
          url: '/api/getplaylistscolombia/' + category,
          type: "GET",
          dataType: 'json'
        }).done(function(results){
          console.log(results);
      ...
      $categoriesTitle.text('Choose from one of the top Categories in Colombia to hear top Playlists');
