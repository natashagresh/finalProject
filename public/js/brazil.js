console.log('connectedbrazil');

$(document).ready(function(){
  
  var $body = $('body');

   $('#brazil').on('click', function(event){
    $.ajax({
      url: '/api/releasesbrazil',
      type: 'GET',
      dataType: 'json'
    }).done(function(results){
        // var $releasesTitle = $('<h2>');
        var releasesTitle = '<h2>New Releases in Brazil</h2>';
  ...
  $('#brazil').on('click', function(event){
    $.ajax({
      ...
      $('#categoriesContainer').on('click', 'p', function() {
        ...
        $.ajax({
          url: '/api/getplaylistsbrazil/' + category,
          type: "GET",
          dataType: 'json'
        }).done(function(results){
          console.log(results);
      ...
      $categoriesTitle.text('Choose from one of the top Categories in Brazil to hear top Playlists');
