console.log('connectedsingapore');

$(document).ready(function(){
  
  var $body = $('body');

   $('#singapore').on('click', function(event){
    $.ajax({
      url: '/api/releasessingapore',
      type: 'GET',
      dataType: 'json'
    }).done(function(results){
        var releasesTitle = '<h2>New Releases in Singapore</h2>';
  ...
  $('#singapore').on('click', function(event){
    $.ajax({
      ...
      $('#categoriesContainer').on('click', 'p', function() {
        ...
        $.ajax({
          url: '/api/getplaylistssingapore/' + category,
          type: "GET",
          dataType: 'json'
        }).done(function(results){
          console.log(results);
      ...
      $categoriesTitle.text('Choose from one of the top Categories in Singapore to hear top Playlists');
